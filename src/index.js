...
function embedFonts(options) {
    return node =>
        fontFaces.resolveAll().then(function(cssText) {
            var styleNode = document.createElement("style")
            var extraStyles = Promise.resolve(
                options.extraStyles
                    ? inliner.inlineAll(
                          options.extraStyles,
            ...
            return extraStyles.then(styleTxt => {
                node.appendChild(styleNode)
                styleNode.appendChild(
                    document.createTextNode(cssText + styleTxt)
                )
                return node
            })
        })
...
else window.domtoimage = domtoimage
